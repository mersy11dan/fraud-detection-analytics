...
# Train/tune 3 models + export comparison artifacts
...
### SHAP explainability (`reports/figures/` and `reports/shap/`)

Generated by `python scripts/run_shap_analysis.py` or `run_shap_explainability_workflow()`:

| File | Description |
|------|-------------|
| `figures/shap_summary_plot.png` | SHAP beeswarm summary |
| `figures/shap_bar_plot.png` | Mean |SHAP| bar chart |
| `figures/shap_waterfall_plot.png` | Single-case fraud score breakdown |
| `figures/shap_dependence_plot.png` | Top-feature dependence plot |
| `shap/shap_top_features.csv` | Top 10 predictors by mean |SHAP| |
| `shap/shap_feature_summary.md` | Stakeholder-friendly feature explanations |
...
**Explainability:** SHAP workflow (`run_shap_explainability_workflow`) explains the best model with summary, bar, waterfall, and dependence plots plus a stakeholder markdown summary in `reports/shap/`.
...
# SHAP explainability for the best model
from src.modeling import run_shap_explainability_workflow
...
- [x] SHAP explainability workflow (summary, bar, waterfall, dependence plots)
- [ ] Modeling pass for `creditcard.csv`

## Next Steps

1. Tune decision thresholds for business-specific precision/recall trade-offs
2. Extend feature engineering and modeling to `creditcard.csv`
3. Compare SMOTE against class-weighted and other resampling strategies
